...
def metric_average(val_tensor):
    dist.all_reduce(val_tensor, async_op=False)
    val_tensor.div_(dist.get_world_size())
...
def create_cosine_lr_schedule_with_window(num_warmup_steps, num_window_steps, num_training_steps, num_cycles=0.5):
    def lr_schedule(current_step: int):
        if current_step < num_warmup_steps:
            ...
            #return 0.5 * float(current_step - num_warmup_steps) / float(num_window_steps)
            return 0.5
