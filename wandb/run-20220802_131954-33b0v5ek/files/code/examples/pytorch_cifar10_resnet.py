...
def initialize():
    ...
    logfile = os.path.join(args.log_dir,
        '{}_{}_ep{}_bs{}_gpu{}_lb{}_{}.log'.format(args.dataset, args.model, args.epochs, args.batch_size, hvd.size(), args.last_batch, args.lr_schedule))
...
def train(epoch, model, optimizer, lr_scheduler, criterion, train_sampler, train_loader, args):
    ...
    if args.verbose:
        ...
        if wandb:
            wandb.log({"train loss": train_loss.avg.item(), "epoch": epoch})
...
def test(epoch, model, criterion, test_loader, args):
    ...
    if args.verbose:
        ...
        if wandb:
            wandb.log({"eval acc": test_accuracy.avg.item(), "epoch": epoch})
